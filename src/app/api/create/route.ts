...
export async function POST(req: Request) {
  try {
    const { total_games } = await req.json()
    const new_game = Number.isFinite(Number(total_games)) ? Number(total_games) : 1
    ...
    if (!analytics) {
      return new Response('Região não encontrada', { status: 404 })
    }
    ...
    if (isSameDay && latest) {
      ...
      const updated = await prisma.metrics.update({
        ...
        data: { total_games: { increment: new_game } },
      })
      return Response.json({ ok: true, mode: 'updated', metricsId: updated.metricsId, total_games: updated.total_games })
    } else {
      // 4) Data diferente: criar novo registro na tabela de métricas
      const created = await prisma.metrics.create({
        data: { analyticsId: analytics?.analyticsId, total_games: new_game },
      })
      return Response.json({ ok: true, mode: 'created', metricsId: created.metricsId, total_games: created.total_games })
    }
  } catch (e) {
    console.error('Erro ao criar/atualizar métricas:', e)
    return new Response('Erro ao processar métricas', { status: 500 })
  }
}